...
from .models import ExchangeMarketType, ExchangeName, OrderSide, OrderType
...
class OrderExecuteRequest(BaseModel):
    ...
    def validate_market_configuration(self) -> "OrderExecuteRequest":
        if self.market_type == "SPOT" and self.leverage != 1:
            raise ValueError("SPOT orders must use leverage=1.")
        if self.order_type == "LIMIT" and self.reference_price is None:
            raise ValueError("LIMIT orders require reference_price.")
        return self
...
class OrderExecuteResponse(BaseModel):
    ...
    side: OrderSide
    order_type: OrderType
    status: Literal["FILLED", "OPEN", "PARTIALLY_FILLED", "CANCELED", "REJECTED", "FAILED"]
    client_order_id: str
